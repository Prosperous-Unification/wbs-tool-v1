...
have to become **enforced contracts, each certified by a kit and served by SQLite** (the memory
source follows behind, D29), one seam (HTTP) that does
...
| D4  | Second adapters as living proof?                                      | **No** second HTTP adapter (Dany: "good idea, overkill for now"). The store kits run against SQLite and the in-memory source, and the memory source is allowed to lag behind (D29). HTTP characterization tests stay **local to the Elysia adapter**; an exported HTTP kit is written the day a second adapter exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| D14 | **New.** How is dependency direction stated across packages?          | **Rings**, as Nx tags, and **every project has exactly one**: `ring:domain` (`domain`, `contracts`, `validation`), `ring:application` (`core`, `conformance`), `ring:adapter` (`store-*`, `runtime-portable`, `auth`, `realtime`, `solver-py`, `observability`, `config`, `be-01`, `fe-01`, `gw-01`, `mcp-01`, and every `tools/*` project). Each ring depends only inward; fe-01 additionally only on `ring:domain` and `runtime:browser` adapters. **Test files are outside the ring constraints** (not the runtime ones): a core test composes core over `@wbs/store-memory`, and that is the one hole, deliberately. `domain` and `contracts` stay **separate Nx projects** because fe-01 and gw-01 import them and a boundary is per project. A totality test asserts one `ring:` and one `runtime:` tag on every project, and one `product:` tag on every project under `apps/` and `libs/` (§3.5 #12).                                                                                                                                                                                                        |
...
| D29 | Must every port method land in both sources in the same PR?           | **No — relaxed by Dany, 2026-09-06.** SQLite is the source of record and a feature PR may prove its store method against SQLite alone (a `.db.test.ts`). The memory source may **lag**: a method it does not implement yet is a stub that throws `NotImplemented(source, port, method)`, listed by name in `store-memory`'s **stub allowlist**, and reported as _not offered_ by `sourceConformance(memory)` rather than run. Nothing passes silently: a stub that is called throws (R5), a stub added without its allowlist line fails the allowlist test, and the report names every skipped case. The allowlist is the debt; it shrinks in slices of its own, and it must be empty the day the browser mode (D17) starts, because that mode runs on the memory source. Reason: the last eleven feature commits on `main` touched 2–4 files each; requiring memory and its kit case in the same PR would make a wire-visible field touch contracts, a core port, a core service, `store-sqlite`, `store-memory`, a kit case and fe-01 — 6+ files in 5 projects — for a second implementation nothing ships on yet. |

## 2 · Target layout

```
libs/domain         @wbs/domain          ring:domain       runtime:isomorphic   vocabulary, tree rules, derivation, the TS schedule engine (a pure function; its Scheduler adapter lives in runtime-portable and boot.ts), saved-plan shape
...
libs/core           @wbs/core            ring:application  runtime:isomorphic
  ...
  compose.test.ts   the "any trigger, any runtime" proof: core over @wbs/store-memory with the runtime-portable adapters, no HTTP (a test file, so outside the ring rule)
...
libs/runtime-portable    @wbs/runtime-portable     ring:adapter      runtime:isomorphic   Digest over crypto.subtle, tsScheduler, in-tab Broadcaster — used by fe-01's future browser mode and by be-01's boot.ts alike
...
libs/wbs/adapters/{store-sqlite,store-memory,runtime-portable,auth,realtime,solver-py,observability,config}  product:wbs  ring:adapter
...
- **Kit admission rule.** A case belongs in a kit if it states behaviour a caller can observe
  ...
  `store-sqlite`. A case whose method the memory source stubs (D29) is **reported as not
  offered**, never silently skipped: `sourceConformance` reads the source's stub allowlist and
  prints the skipped cases by name. Negatives: a stub without its allowlist line → the
  allowlist test in `store-memory` is red; a kit case run against a stub → fails on the
  `NotImplemented` throw, not on a wrong answer.
- Promoting the fixtures: each `inMemoryX` is tightened until the kit passes or the method is
  on the allowlist (D29) — the promotion slice records which; `rows` /
...
adapters live in `boot.ts`; the portable adapters live in `libs/runtime-portable`. The "any
trigger, any runtime" proof is `libs/core/src/compose.test.ts`: test files are outside the ring
rule (§2), so it composes core over `@wbs/store-memory` with the `runtime-portable` adapters from
...
5. One kit per port assembled from the `.db.test.ts` files under the admission rule, written
   ...
   `sourceConformance` as their composition with a report naming the kits that ran and the
   cases skipped as not offered; SQLite green with **no** stubs; memory source tightened until
   green or stubbed onto the allowlist (D29), the allowlist test watched failing on a stub
   with no line.
...
1. Packages with `project.json`, ring and runtime tags on **every** project — no project
   ...
   `libs/runtime-portable` and `libs/conformance`, `tsconfig`, `typecheck` running `tsc --build --force` on the
...
4. `composeServices({ source, runtime, shared })` and the four use-case entrypoints; the "any
   ...
   memory source with the `runtime-portable` adapters for `Digest` and `Scheduler`, a batch-owned

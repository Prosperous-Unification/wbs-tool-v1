...
Keep the discriminator relationship explicit. A single audited generic dispatch helper is permitted to bridge TypeScript's indexed-union limitation, with an adjacent boundary comment and compile-negative fixtures proving wrong-kind input and wrong-kind response cannot be bound. Do not distribute casts into individual handlers or accept an untyped catch-all. Adding a temporary definition with no normalizer or handler must fail typecheck at the two respective records.

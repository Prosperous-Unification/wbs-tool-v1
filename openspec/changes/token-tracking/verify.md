...
| F7     | the `before === null` inverse struck from `setMeasure`, leaving `set_measure … value: before ?? 0` | `1f6dfd0` | `undoes a first recording back to absence, not to zero` **and** `undoes a first recording of one metric without touching the pair's others` | a stored 0 where nobody said anything — 14 pass, **2** fail |

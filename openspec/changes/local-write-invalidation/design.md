...
The Section 1 baseline at `c61b370d` has these exact coordinator paths:

| Coordinator path                 | Current callers and API boundaries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `usePlanRead.run`                | `use-plan-structure.ts` (create, move, duplicate, remove and patch work item); `use-plan-fields.ts` (work-item fields and status); `use-estimate-drafts.ts` (set/clear estimate); `use-reference-sets.ts` (work-item directory sets, create-and-attach, person assignment and estimate method); `use-plan-dependencies.ts#pickDependency`; `plan-toolbar.tsx` (freeze/unfreeze and start date); `plan-columns/actions.tsx` (row unfreeze); `plan-columns/depends.tsx` (remove dependency); `wbs-table.tsx` (arrange, optimization settings/retry, remove dependency and row unfreeze). `plan-live.ts` carries this event-only capability to cells. |
| `usePlanRead.refreshOrMarkStale` | The `run` success/recovery path; `use-plan-dependencies.ts#dependOn` after its deliberately partial dependency loop; every `ProjectSettingsModal` `onChanged` callback in `plan-toolbar.tsx` after capacity, bands, steps, dependency reach, arithmetic or optimization writes; and the optimization failure retry control in `wbs-table.tsx`. These retain the existing full-scope default in Section 1.                                                                                                                                                                                                                                          |
| `usePlanRead.runMarkerWrite`     | The four calendar call sites in `wbs-table.tsx`: create, rename, recolor and delete. Its marker-only success and modeled-refusal reread contract remains separate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `usePlanRead.stepStack`          | Toolbar Undo/Redo in `plan-toolbar.tsx` and keyboard Undo/Redo in `use-plan-keyboard.ts`. Both retain full recovery after the stack answer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

R10's interfaces remain unchanged around that inventory: `PlanRowReadings` and
`PlanRenderRow` stay immutable render input, `PlanLiveValues.run` remains an event-only
capability, `usePlanRead` still receives `cellCards`, and `usePlanStructureEffects` still
receives the `attachCell` ref used by `FocusIntent.land`.

...
- [ ] 1.1 The three tests TASK-219 excised come back verbatim into
...
- [ ] 1.2 Watch all three red first, against the unordered select on `main`:
...
- [ ] 1.3 `ORDER BY work_item.id` on `listByProject`'s work-item select, and the
...
- [ ] 1.4 The three tests go green and the rest of the be-01 suite stays green.
...
- [ ] 2.1 `index('work_item_project_id_id').on(t.projectId, t.id)` in
      `schema.ts` beside `work_item_siblings`, with the JSDoc saying it exists to
      serve slice 1's `ORDER BY` rather than a `WHERE`.
- [ ] 2.2 One migration folder, `<stamp>_add_work_item_read_order_index`: the
...
- [ ] 2.3 The measurement, in the migration comment and in `verify.md`, taken the
...
- [ ] 3.1 A read-only probe over the live database that counts projects holding
...
- [ ] 3.2 Pre/post fixtures for each affected project: the plan as it schedules
...
- [ ] 3.3 The announcement itself lands with the PR, so the movement is stated
...
- [ ] 4.1 `bunx nx run-many -t test lint typecheck build` and

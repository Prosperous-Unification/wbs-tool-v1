...
Active TASK-220: `change/optimized-scheduler-coordinator`, PR 216; coordinator admission, restart-safe FIFO launch, generation reuse/eviction, atomic `schedule_optimized` replay records, and the four-way 6.9 negative matrix are landed. Next close the remaining owned slot-fencing and empty-project proofs; failure events and Retry remain deferred. Do not merge or install the host service.

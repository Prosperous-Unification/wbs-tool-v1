...
Active TASK-220: `change/optimized-scheduler-coordinator`, PR 216; coordinator admission, restart-safe FIFO launch, generation reuse/eviction, atomic `schedule_optimized` replay records, the four-way 6.9 negative matrix, empty-plan bypass, and the seven-state plan-read DTO are landed. Next reconcile the remaining owned slot-fencing proof; failure events and Retry remain deferred. Do not merge or install the host service.

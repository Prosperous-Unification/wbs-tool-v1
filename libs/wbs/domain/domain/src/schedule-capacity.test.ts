...
/**
 * A row, carrying only what the engine reads.
 *
 * Until 2026-09-02 this built a full fifteen-field `WorkItem`, and 35 call sites
 * across this file and `schedule-joint-capacity.test.ts` passed
 * `{ maxParallel: 3 }` or `{ serviceTeamId: PLATFORM }` into it as though that
 * were the setup for the assertion below. It was not. The engine reads neither
 * field off a row — `width` and `poolIds` on the **slice** are the adapter's
 * reading, exactly as the `slice` factory below says — so those arguments named
 * a cause that had no effect, and an agent changing one would watch nothing
 * move.
 *
 * They are gone, and {@link PlannedRow} is now the type, so they cannot come
 * back: an override object naming `maxParallel` is a compile error. All 149
 * engine cases stayed green through their removal, which is what proves they
 * were decorative.
 */
let position = 0;
const item = (
...
describe('width — effort compressed across the people on it', () => {
  ...
  it('holds a subnormal and a tiny estimate at width 1 and at width 1000', () => {
    ...
    const tiny = 5e-10;
    const wide = schedule([item('a')], [], [slice('a', DEV, tiny, { width: 1000 })]);
...
describe('capacity — a team is a pool of slots', () => {
  ...
  it('skips a gap it cannot fit inside and waits for the whole window', () => {
    ...
    const rows = [
      item('early', { priority: 1 }),
      item('late', { priority: 2 }),
      item('wide', { priority: 3 }),
    ];
    const slices = [
  ...
  it('lets a block run through the instant another hands its slot over', () => {
    ...
    const rows = [
      item('held', { priority: 1 }),
      item('next', { priority: 2 }),
      item('through', { priority: 3 }),
    ];
    const slices = [
  ...
  it('answers the same however the rows arrived, so no entry order decides anything', () => {
    ...
    const slicesFor = (order: readonly PlannedRow[]): Slice[] =>
  ...
  it('gives a slice nobody has estimated no reservation and no wait', () => {
    ...
    const rows = ['z', 'a', 'b'].map((id) => item(id));
    const slices = [
...
describe('float — the blocking set is the whole set', () => {
  it('reports no float on a block whose slack another block’s finish is holding', () => {
    ...
    const rows = [item('A'), item('B'), item('X')];
    const slices = [
...
describe('determinism and refusals', () => {
  it('answers the same for a shuffled copy of the same plan', () => {
    ...
    const rows = ['a', 'b', 'c', 'd'].map((id) => item(id));
    const slicesFor = (order: readonly PlannedRow[]): Slice[] =>
      order.map((row) =>
  ...
  it('terminates on a plan whose every slice is on one full pool', () => {
    const rows = Array.from({ length: 30 }, (_, at) => item(`w${String(at)}`));
...
describe('the scan, instrumented', () => {
  it('visits a bounded number of pool events on a plan the size of a real one', () => {
    ...
    const rows = Array.from({ length: 200 }, (_, at) => item(`w${String(at)}`));

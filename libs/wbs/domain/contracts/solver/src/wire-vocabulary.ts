...
export const NEIGHBOUR_VOCABULARIES: readonly Vocabulary[] = [
  {
    ...
    source:
      "libs/wbs/domain/domain/src/schedule.ts:31 — `interface Slice`, the domain input tuple, which is NOT the wire slice: the wire's `key` folds the first two, and `durationUnits`, `priorityWeight`, `notBeforeUnits`, `deadlineUnits` and `workItemIsMilestone` are derived rather than carried in",
  },
  {
    name: 'canonical-row',
    members: new Set(['id', 'parentId', 'position', 'frozenNumber', 'priority']),
    source:
      'libs/wbs/domain/domain/src/canonical-schedule-input.ts:184-188 — the hashed `PlannedRow` facts, as the canonicalizer projects them',
  },
  {
    name: 'fencing',

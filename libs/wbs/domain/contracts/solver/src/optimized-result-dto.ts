 ...
 *
 * **Why it is here and not in `libs/wbs/domain/domain` beside 4.12's schedule codec.**
 * The envelope needs both halves: `encodeSchedule` from `@wbs/domain` and the
 * stage-status vocabulary the wire schema generates. `libs/wbs/domain/contracts/solver`
 ...
 * (`libs/wbs/domain/domain/src/schedule.ts:539-541`), so the mandated width-5 floor row's
 ...
 *
 * The guard lives in `libs/wbs/domain/domain` and answers

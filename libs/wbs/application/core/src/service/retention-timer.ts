...
export interface RetentionTimerOptions {
   ...
   *
   * Proof: removing `intervals: systemInterval` from the production composition
   * failed be-01:typecheck with TS2741 at its RetentionTimer construction
   * (2026-09-09).
   */
  intervals: Intervals;
  /**
   * The clock the history's cutoff is measured from. Required so core owns no runtime default.
   * Proof: removing `now: clock.now` from the production composition failed
   * be-01:typecheck with TS2741 at its RetentionTimer construction (2026-09-09).
   */
  now: () => number;
}

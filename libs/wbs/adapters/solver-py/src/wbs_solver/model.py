...
------------------------------------------------------------------
`libs/wbs/domain/contracts/solver/src/revalidate-solver-result.ts` is the Bun half of the
...
def build_model(request: Mapping[str, Any]) -> BuiltModel:
    ...
    for entry in slices:
        ...
        # (`libs/wbs/domain/domain/src/on-time.ts`, via `lastWorkdayOf`) reads that

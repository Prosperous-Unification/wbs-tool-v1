...
1. `__version__` against the golden request corpus, which spends the current
   ...
   `libs/wbs/domain/contracts/solver/src/wire-contract-version.test.ts`, which asserts the
...
FIXTURES = REPO_ROOT / "libs" / "wbs" / "domain" / "contracts" / "solver" / "fixtures" / "request"

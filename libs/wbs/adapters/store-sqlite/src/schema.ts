...
export const workItem = sqliteTable(
  'work_item',
  {
     ...
     *
     * **Read by no scheduling code.** `libs/wbs/domain/domain/src/schedule.ts` does not select it,
     ...
     * `apps/wbs/fe-01` consumes the number yet. Said plainly because the paragraph
     ...
     * in `libs/wbs/domain/domain/src/schedule.ts` for where it is asked.
     *
     ...
     * in `libs/wbs/domain/domain/src/effective-team.ts` — and each of those leaves' slices
     ...
     *
     * **Nothing a date reads.** `libs/wbs/domain/domain/src/schedule.ts` has an empty diff in the
 ...
 * adds this table has an empty diff on `libs/wbs/domain/domain/src/schedule.ts` and `libs/wbs/domain/domain`.
 *
 ...
 * `libs/wbs/domain/domain/src/schedule.ts` has an empty diff in the change that adds it, watched by
 ...
 * `libs/wbs/domain/domain` has patterns for, so an unseeded table means a pasted GitHub URL
 ...
 * `libs/wbs/domain/domain/src/schedule.ts` has an empty diff in the change that adds this, watched
 ...
 * the future SDLC integration read, and `libs/wbs/domain/domain/src/schedule.ts` has an empty diff
 ...
 * the successor (`libs/wbs/domain/domain/src/schedule.ts`'s `reachedSliceOf`). The reach is stored
 ...
 * solver wire, the convention `libs/wbs/domain/contracts/solver/src/wire-types.ts` states
 * for its own: a bare union is erased before a test can see it, so the drift
 * guard needs a value to read. `libs/wbs/domain/contracts/solver/solver-wire.v1.json` is
 * the normative definition of `objective`; this file does not import it because
 * `libs/wbs/domain/contracts/solver` has no path alias, so agreement is asserted by test
 ...
 * `libs/wbs/domain/domain/src/schedule.ts` reads this table, and slice 5 of

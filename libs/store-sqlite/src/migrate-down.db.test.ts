...
describe('readMigrationFolders', () => {
  it('throws when the migration folder is absent', () => {
    const missing = join(tmpdir(), `wbs-missing-migrations-${crypto.randomUUID()}`);
    // Proof: converting a missing folder to [] made this return without throwing.
    expect(() => readMigrationFolders(missing)).toThrow(/ENOENT/);
  });

  it('throws when the migration folder is unreadable', () => {
    const dir = mkdtempSync(join(tmpdir(), 'wbs-unreadable-migrations-'));
    chmodSync(dir, 0o000);
    try {
      expect(() => readMigrationFolders(dir)).toThrow(/EACCES|permission denied/i);
    } finally {
      chmodSync(dir, 0o700);
      rmSync(dir, { recursive: true, force: true });
    }
    // Proof: catching the directory read and returning [] made this fail on
    // `Expected function to throw`, with `Received function did not throw`.
  });

  it('throws when a migration directory is unreadable', () => {
    const dir = mkdtempSync(join(tmpdir(), 'wbs-unreadable-migration-directory-'));
    const migration = join(dir, '20260101000000_unreadable');
    mkdirSync(migration);
    chmodSync(migration, 0o000);
    try {
      expect(() => readMigrationFolders(dir)).toThrow(/EACCES|permission denied/i);
    } finally {
      chmodSync(migration, 0o700);
      rmSync(dir, { recursive: true, force: true });
    }
    // Proof: filtering with `existsSync(migration.sql)` returned false on EACCES,
    // so this failed on `Expected function to throw`, with `Received function did not throw`.
  });

  it('throws when a required migration script is unreadable', () => {
    const dir = mkdtempSync(join(tmpdir(), 'wbs-unreadable-migration-'));
    const migration = join(dir, '20260101000000_unreadable');
    mkdirSync(migration);
    writeFileSync(join(migration, 'migration.sql'), 'CREATE TABLE t (id text);');
    const down = join(migration, 'down.sql');
    writeFileSync(down, 'DROP TABLE t;');
    chmodSync(down, 0o000);
    try {
      // Proof: treating an unreadable down.sql as empty made this throw the
      // modeled empty-script error instead of the filesystem EACCES.
      expect(() => readMigrationFolders(dir)).toThrow(/EACCES|permission denied/i);
    } finally {
      chmodSync(down, 0o600);
      rmSync(dir, { recursive: true, force: true });
    }
  });

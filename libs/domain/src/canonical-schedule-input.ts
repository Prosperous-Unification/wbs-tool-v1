 ...
 * empty map and the entry canonicalizes to `[]`. That empty state is the
 * *proved* state rather than a placeholder: 1.6's and 4.3's no-op proof
 * requires the seventh argument to leave every golden corpus case
 * byte-identical, and `fast-golden-corpus.test.ts` now runs that comparison on
 * the real argument rather than on the one that happened to occupy the slot.
 */

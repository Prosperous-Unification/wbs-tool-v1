...
test.describe('the table, measured by a browser', () => {
  ...
  test('a toolbar panel closes when the pointer goes down outside it', async ({ page }) => {
    ...
    // rather than positioned popovers, and a native `<details>` closes on its
    // summary and on nothing else — so an open panel stayed over the plan while
    // the reader went back to the table.
    //
    // Export is here too though Dany named three: it is the same element with
    // the same manners, and leaving one of four behaving differently is worse
    // than the original fault.
    for (const panel of ['Filters', 'Views', 'Columns', 'Export'] as const) {
      const summary = page.getByText(new RegExp(`^${panel}`), { exact: false }).first();
      await summary.click();
      const open = page.locator('details[open]');
      await expect(open, `${panel} did not open`).toHaveCount(1);

      // On the table, which is where a reader's next click actually goes, and
      // far from any panel. `mouse.click` rather than a locator's, so this is a
      // real hit test at a point rather than an element Playwright chose.
      const table = await page.locator('tbody tr').first().boundingBox();
  ...
  test('a step’s figure lands at one x whether or not the row is assigned', async ({ page }) => {
    // Dany, 2026-08-31, with a screenshot: "when no assignee the resulting
    // estimate of a step is off". Measured on his own plan before the fix: `· 2`
    // stood at **1143.75** on the seven rows carrying an assignee and at
    // **1172.32** on the one without — 28.57px out, which is `· VS` (24.57) plus
    // its 4px margin.
    //
    // The mechanism was the trio box's `flex: 1`: it ate every spare pixel, so
    // the figure and the assignee packed against the cell's right edge and the
    // figure's x became a function of *whether somebody is assigned*.
    //
    // **Only a browser can see this.** jsdom lays out no flex line, so both rows
    // report the same nothing.
    // `seedPlan`'s two rows are all this needs: one of them takes an assignee
    // and the other does not, which is the whole comparison.
    for (const [row, estimate] of [
      ['010', '1/2/3'],
      ['020', '2/3/4'],
    ] as const) {
      const cell = page.getByLabel(`Dev estimate for ${row}`);
      await cell.fill(estimate);
      await cell.blur();
      await expect(cell).toHaveValue(estimate);
    }

    // **The other half of Dany's rule, asserted while it is still true: an
    // unstaffed plan reserves nothing.** "If there is no assignees on any work
    // item, then everything is aligned vertically without assignee, if there is
    // at least one assignee, then every row moves." Nobody has been assigned
    // yet, so no cell may be paying for a slot.
    //
    ...
    //
    // Proof: `anyAssigneeOn` made to answer `true` always, this failed on
    // `expected 4 to be 0` — every cell reserving a slot on a plan with nobody
    // on it. Watched in Chromium, 2026-08-31.
    expect(
      await page.locator('[data-folded-assignee-slot]').count(),
      'an unstaffed plan reserved the assignee slot anyway',
    ).toBe(0);
    ...
    // carries an assignee and one that carries none.
    const rowOf = (number: string) =>
      page.getByLabel(`Name of ${number}`).locator('xpath=ancestor::tr[1]');
    await expect(rowOf('010').locator('[data-folded-assignee]')).not.toHaveCount(0);
    await expect(
      rowOf('020').locator('[data-folded-assignee]'),
      '020 must carry no assignee for this comparison to mean anything',
    ).toHaveCount(0);

    const figureX = async (row: string): Promise<number> => {
      const at = await page
        .getByLabel(`Dev estimate for ${row}`)
        .locator('xpath=ancestor::td[1]')
        .locator('[data-folded-final]')
        .boundingBox();
      if (at === null) throw new Error(`row ${row} draws no derived figure`);
      expect(at.width, `row ${row}'s figure has no width`).toBeGreaterThan(0);
      return at.x;
    };

    // Proof: `ASSIGNEE_SLOT_PX` taken off the assignee **and** its spacer, so
    // the slot is the initials' own width again — this failed on
    // `Expected: 844.296875 · Received: 868.875`, **24.578125px apart**, which
    // is exactly `· VS`. (The spacer's 4px margin survives that injection, so
    // the gap is 24.58 rather than the 28.58 the original fault measured on
    // Dany's live plan — 1143.75 against 1172.32 — where there was no spacer at
    // all.) Watched in Chromium, 2026-08-31.
    //
    // At the **declared** column width, which is the point: an earlier attempt
    // sized the box instead (`flex: 0 1 59px`) and had to widen the column
    // before it could assert anything, because at 96px the box's shrink took
    // the difference and how much it shrank depended on the assignee again —
    // 18.48px apart, measured. A reserved slot needs no such staging.
    expect(await figureX('020'), 'the figure moves when nobody is assigned').toBeCloseTo(
      await figureX('010'),
      0,
    );
  });

/**
 * Everything this browser remembers about how one project's plan is laid out —
 * the eight `localStorage` triples the table reads at mount and writes when a
 * control is used, and the shape checks that turn each stored claim into a fact.
 *
 * Extracted from `wbs-table.tsx` by W4-4 (`docs/2026-09-02-refactoring-plan.md`)
 * unchanged: same names, same guards, same "drop the key rather than throw"
 * posture, which is stated once per triple where the reasoning differs. What
 * makes this a concept rather than a utility bag is the shared rule — every
 * value here is **one reader's answer about their own screen**, per browser,
 * and be-01 is never told about any of it.
 */

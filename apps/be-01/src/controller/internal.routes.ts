...
import { bind, type RequestMetadata } from '../http/endpoint';
...
export function internalRoutes(deps: InternalDeps) {
  return [
    ...
    bind(resumeInternal, async ({ body, request }) => ({
      ok: true,
      status: 200,
      body: await deps.onResume(
        Object.fromEntries(Object.entries(body.resume_points)),
        contextFrom(request, body.trace_id),
      ),
    })),
  ] as const;
}

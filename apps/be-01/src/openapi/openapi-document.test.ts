...
describe('the committed OpenAPI document', () => {
  ...
  /**
   * The eight self-checking bodies, held to declaring the three media types the
   * app actually accepts.
   *
   * These are the other half of the check above, and the reason it stays honest
   * once this one exists. Both helpers write the caveat sentence; only
   * `checkedBody`'s ends in **422 `invalid_body`**, and that sentence — not the
   * media-type count — is what proves a body was written here rather than
   * generated by Elysia from a `t.Object`. So the count above is a signature for
   ...
   * these eight were `t.Object(...)` schemas and Elysia derived

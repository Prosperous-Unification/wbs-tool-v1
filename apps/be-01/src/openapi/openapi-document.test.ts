...
describe('the committed OpenAPI document', () => {
  ...
  /**
   * The ten self-checking bodies, held to declaring the three media types the
   * app actually accepts.
   *
   * These are the other half of the check above, and the reason it stays honest
   * once this one exists. The hand-parsed helper's caveat ends in **400 with a
   * code**, and every self-checking one ends in **422** — `checkedBody`'s in
   * `invalid_body`, `tableRefusedBody`'s in the refusing route's own
   * `error`/`field` pair. That sentence — not the media-type count — is what
   * proves a body was written here rather than generated by Elysia from a
   * `t.Object`, so this clause asserts the `answers 422` all ten share and
   * leaves the tail to each helper. So the count above is a signature for
   * the two command routes alone, and this list carries its own.
   *
   * **Three rather than one, because three is what the app answers.** On `main`
   * these ten were `t.Object(...)` schemas and Elysia derived
   * `application/json`, `application/x-www-form-urlencoded` and
   * `multipart/form-data` for each. Moving the checks into the handlers dropped
   * the declarations while the app went on accepting all three — chunk 21 made
   * the in-process binder read form bodies for the same reason, and `decodeBody`
   * carries the measurement. A document that names one media type for a route
   * that takes three tells a client its form post will be refused when it will
   * be served, which is the same class of lie as the "answers 400" sentence six
   * operations carried two chunks ago.
   *
   * `PATCH /api/projects/{id}` declares them with a nested `pertWeights` object
   * in its schema, which no form encoding can express. That is not new and not
   * this branch's to decide: `main` declared exactly this, the refactor's claim
   * is that it changed no API, and narrowing a published body to JSON belongs to
   * whoever wants it, with the clients told. Same argument, same words, as the
   * 405 the in-process binder gave up.
   */
  it('declares all three accepted media types on every self-checking body', async () => {
    const document = await documentFromApp(testApp());
    const selfChecking: readonly [string, string][] = [
      ['/api/auth/register', 'post'],
      ['/api/auth/login', 'post'],
      ['/api/projects', 'post'],
      ['/api/projects/{id}', 'patch'],
      ['/api/projects/{id}/steps', 'post'],
      ['/api/projects/{id}/steps/{stepId}', 'patch'],
      ['/api/projects/{id}/saved-plans', 'post'],
      ['/api/saved-plans/{id}', 'patch'],
      // TASK-235's two, ported onto the route shape when this branch merged the
      // refactor. They refuse with a row of the marker refusal table rather
      // than `invalid_body`, which is why they carry `tableRefusedBody`'s
      // caveat and not `checkedBody`'s.
      ['/api/projects/{id}/calendar-markers', 'post'],
      ['/api/projects/{id}/calendar-markers/{markerId}', 'patch'],
    ];
    const paths = document['paths'] as Record<string, Record<string, unknown> | undefined> | null;
    for (const [path, method] of selfChecking) {
      const operation = paths?.[path]?.[method] as
        | {
            requestBody?: {
              description?: string;
      ...
      expect(Object.keys(content).sort(), `${method} ${path}`).toEqual([
        'application/json',
        'application/x-www-form-urlencoded',
        'multipart/form-data',
      ]);
      // One body, three envelopes. A per-media-type schema would be a second
      // shape to keep in step with the handler's checks, and the handler has
      // only one.
      const json = JSON.stringify(content['application/json']?.schema);
      expect(JSON.stringify(content['application/x-www-form-urlencoded']?.schema)).toBe(json);
      expect(JSON.stringify(content['multipart/form-data']?.schema)).toBe(json);
    }
  });
});

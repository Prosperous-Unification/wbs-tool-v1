 ...
 * `dist/apps/wbs/fe-01` with **`apps/wbs/fe-01/Caddyfile`** — the exact file
 * `apps/wbs/fe-01/Dockerfile` copies to `/etc/caddy/Caddyfile`, mounted rather
...
if (!existsSync(join(repoRoot, 'apps', 'wbs', 'fe-01', 'playwright.packaged.config.ts'))) {
  throw new Error(
    `The packaged gate must be run from the workspace root; this is ${repoRoot}. ` +
      `Use \`nx run wbs-fe-01:e2e-packaged\`, which builds first.`,
  );
}

const site = join(repoRoot, 'dist', 'apps', 'wbs', 'fe-01');
if (!existsSync(join(site, 'index.html'))) {
  ...
  throw new Error(`${site} holds no index.html; run \`bunx nx run wbs-fe-01:build\` first.`);
}
...
export default defineConfig({
  testDir: join(repoRoot, 'apps', 'wbs', 'fe-01', 'e2e-packaged'),
  outputDir: join(repoRoot, 'apps', 'wbs', 'fe-01', 'test-results-packaged'),
  fullyParallel: false,
  workers: 1,
  retries: 0,
  timeout: 60_000,

 ...
 * proxies nowhere. `bun run dev:setup` writes one; `nx run wbs-fe-01:e2e` runs that
...
function edgeRoutes(mode: string): Record<string, ProxyOptions> {
  ...
  if (!backend || !gateway) {
    throw new Error(
      `apps/wbs/fe-01/.env must set VITE_BE_URL and VITE_GW_URL; got ` +
...
export default defineConfig(({ command, mode }) => ({
  ...
  // `{projectRoot}/vite.config.[jt]s`, so `nx run wbs-fe-01:build` answered from
  ...
  resolve: {
    alias: {
      ...
      //
      // - `rootDir: "src"` back: `nx typecheck wbs-fe-01` fails on
      //   `wbs-table.tsx(9,33): error TS6059: File '…/libs/wbs/domain/domain/src/
      //   workday.ts' is not under 'rootDir' '…/apps/wbs/fe-01/src'`.
      // - neither `rootDir` nor `noEmit`: typecheck passes and writes
      //   `workday.js` and `workday.js.map` into `libs/wbs/domain/domain/src` — a build

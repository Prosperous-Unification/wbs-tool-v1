...
describe('the app and the run resolve the same modules', () => {
  it('maps every alias to the namespaced source file in both configs', async () => {
    ...
    // Proof: with both maps resolving their legacy `../../libs/*` targets,
    // this failed with every shared alias under `apps/libs/*` instead of
    // `libs/wbs/*`. Restoring only Vite's old workday target also made the
    // actual `wbs-fe-01:build` fail on `UNLOADABLE_DEPENDENCY` from
    // completion-prompt.tsx (2026-09-14).
  ...
  it('writes the build to the namespaced application artifact root', () => {
    ...
    // received path instead of `../../../dist/apps/wbs/fe-01` (2026-09-14).

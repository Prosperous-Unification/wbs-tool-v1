...
describe('fe-01 lint inputs', () => {
  it.each(['lint', 'lint:fast'])('includes every root TypeScript source in %s', (target) => {
    ...
    // Proof: omitting moved `vite.config.ts` made the real `wbs-fe-01:lint`
    // target pass with a deliberate unused variable still in that file, while
    // this case failed naming exactly `apps/wbs/fe-01/vite.config.ts`
    // (2026-09-14).

 ...
 * `apps/wbs/fe-01/src`.** Text that arrives at render as a value — a toast's
...
/**
 * `apps/wbs/fe-01`, which is where the suite runs.
 *
 * The cwd comes from the `cwd` of the `test` and `test:unit` targets in
 * `apps/wbs/fe-01/project.json`, **not** from either config — neither
 ...
 * this is `apps/wbs/fe-01`; invoked by hand from elsewhere the walk below finds

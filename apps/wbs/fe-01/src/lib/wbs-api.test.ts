...
describe('what a refused step change says', () => {
  it('has a sentence for every code stepController answers with', () => {
    // The list is `statusFor` in `apps/wbs/be-01/src/controller/step.controller.ts`
...
describe('read ownership across API lifetimes', () => {
  ...
  it.each(['finishDays', 'sameOrderAsFast'] as const)(
    ...
    async (missing) => {
      ...
      // `libs/wbs/domain/contracts/src/http/work-item-response.ts`, the first case failed

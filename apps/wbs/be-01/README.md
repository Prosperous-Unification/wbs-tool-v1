...
- **`controller/`** — framework-free endpoint bindings against shared shapes.
...
```sh
bunx nx run wbs-be-01:test:unit  # ~490 cases, no database, ~11s
bunx nx run wbs-be-01:test:store # ~620 cases against real SQLite, ~45s
bunx nx run wbs-be-01:test       # both
```
